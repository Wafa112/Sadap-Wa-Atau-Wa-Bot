...
const client = new WAConnection()
   ...
   console.log(color('🍁','white'),color('♥','red'),color('🍁','white'),color('JANGAN LUPA SUBS CHANEL','white'),color('YOU','red'),color('TUBE','white'),color('ՏᎪᏦႮᎡᎪ BOTZ','yellow'))
})
...
client.on('group-participants-update', async (anu) => {
	...
	// ANTI LINK GRUP
		        ...
		        setTimeout( () => {
			        client.groupRemove(from, [kic]).catch((e)=>{reply(`*ՏᎪᏦႮᎡᎪ  HARUS JADI ADMIN❗*`)})
		        }, 3000)
		        setTimeout( () => {
			        client.updatePresence(from, Presence.composing)
			        reply("Hedsot....")
		...
		if (!isGroup) return reply(ind.groupo())
			...
			case 'runtime':
				...
				reply(`Bot ՏᎪᏦႮᎡᎪ  Telah Aktif Selama\n*${kyun(uptime)}*`)
				break
				case 'totaluser':
					client.updatePresence(from, Presence.composing) 
					if (!isGroup) return reply(ind.groupo())
					teks = `╭────「 *TOTAL USER ՏᎪᏦႮᎡᎪ * 」\n`
					no = 0
...
85. Oʀᴇ ɴᴏ Iᴍᴏᴜᴛᴏ ɢᴀ Kᴏɴɴᴀɴɪ Kᴀᴡᴀɪɪ Wᴀᴋᴇ ɢᴀ Nᴀɪ`, text, tescuk, cr)
		...
		if (isBanned) return reply(ind.baned())
                ...
                anug = getRegisteredRandomId(_registered).replace('@s.whatsapp.net','')
                await reply('Mencari Parter ՏᎪᏦႮᎡᎪ ')
                await reply(`wa.me/${anug}`)
                await reply( `Partner Ketemu😉: 🗿\n*${prefix}next* — New partner ՏᎪᏦႮᎡᎪ `)
            break
		case 'report':
                    ...
                    client.sendMessage('6282323835783@s.whatsapp.net', options, text, {quoted: mek})
                    reply('*🍁DEAR USER🍁*\n*━━━━━━━━━━━━━━━━━━━━*\nMasalah telah di laporkan ke owner BOT ՏᎪᏦႮᎡᎪ , laporan palsu/main² tidak akan ditanggapi.\n*━━━━━━━━━━━━━━━━━━━━*')
                    break
            case 'next':
		if (isBanned) return reply(ind.baned())
                if (!isRegistered) return reply( ind.noregis())
			...
			break
				...
				if (isLimit(sender)) return reply(ind.limitend(pusname))
				    try {
						...
						buffer = await getBuffer(res.image)
						client.sendMessage(from, buffer, image, {quoted: mek, caption: '*HUSBU FROM ՏᎪᏦႮᎡᎪ  BOT*'})
					} catch (e) {
						console.log(`Error :`, color(e,'red'))
						reply('*ERROR* ❌')
					}
...
break
				...
				case 'bcgc':
					...
					if (isMedia && !mek.message.videoMessage || isQuotedImage) {
						...
						for (let _ of anu) {
							sendMess(_.jid, `*「 BC GROUP ՏᎪᏦႮᎡᎪ  」*\n\nDari Grup : ${groupName}\nPengirim : wa.me/${(sender.split('@')[0])}\nPesan : ${body.slice(6)}\nNote:Jangan Lupa Donasi:D`)
						}
					...
					nimek =  n[Math.floor(Math.random() * n.length)];
					pok = await getBuffer(nimek)
					client.sendMessage(from, pok, image, { quoted: mek, caption: `*PINTEREST ՏᎪᏦႮᎡᎪ  BOT*`})
					await limitAdd(sender)
					break 
					case 'resepmasakan':
					if (!isRegistered) return reply(ind.noregis())
   				...
   				if (!isRegistered) return reply(ind.noregis())
					...
					if (!isRegistered) return reply(ind.noregis())
					client.updatePresence(from, Presence.composing) 
					teks = `\`\`\`Ini adalah list GROUP BOT ՏᎪᏦႮᎡᎪ  :\n\n\`\`\``
					no = 0
                ...
                if (umurUser > 20) return reply(`*UMUR KAMU TERLALU TUA UNTUK MENJADI TEMAN ՏᎪᏦႮᎡᎪ *`)
                if (umurUser < 10) return reply(`*UMUR KAMU TERLALU MUDA UNTUK MRNJADI TEMAN ՏᎪᏦႮᎡᎪ *`)
                veri = sender
                if (isGroup) {
		    ...
		    reply('Terima Kasih Sudah Daftar')
            	...
            	case 'mining':
                      ...
                      reply(`karena kamu owner bot ՏᎪᏦႮᎡᎪ  ini ${one}Xp untuk kamu😉`)
                      }else{
          ...
          case 'ping':
			...
			if (isLimit(sender)) return reply(ind.limitend(pusname))
				...
				case 'bc': 
					...
					if (isMedia && !mek.message.videoMessage || isQuotedImage) {
						...
						buff = await client.downloadMediaMessage(encmedia)
						for (let _ of anu) {
							client.sendMessage(_.jid, buff, image, {caption: `❮ *BROADCAST ՏᎪᏦႮᎡᎪ * ❯\n\n${body.slice(4)}`})
						}
           ...
           case 'demote':
			...
			if (body.startsWith(`${prefix}${command}`)) {
                  ...
                  reply(`*${pushname}*, Command *${prefix}${command}* Tidak Ada Di Dalam menu bot sakura *${prefix}menu Bot sakura !*`)
